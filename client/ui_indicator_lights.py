...
class IndicatorLightsDialog(QDialog, indicator_lights_ui.Ui_IndicatorLightsDialog):
    ...
    def refresh_status_from_buffer(self):
        ...
        self.push_button_scroll_lock.setChecked(self.keyboard_indicator_lights.scroll_lock)

    def refresh_status_from_ui(self):
        if self.push_button_num_lock.isChecked() == self.keyboard_indicator_lights.num_lock:
            self.keyboard_indicator_lights.num_lock = self.push_button_num_lock.isChecked()
            self.lock_key_clicked_signal.emit("num_lock")
        if self.push_button_caps_lock.isChecked() == self.keyboard_indicator_lights.caps_lock:
            self.keyboard_indicator_lights.caps_lock = self.push_button_caps_lock.isChecked()
            self.lock_key_clicked_signal.emit("caps_lock")
        if self.push_button_scroll_lock.isChecked() == self.keyboard_indicator_lights.scroll_lock:
            self.keyboard_indicator_lights.scroll_lock = self.push_button_scroll_lock.isChecked()
